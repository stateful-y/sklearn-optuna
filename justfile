# Available commands for Sklearn-Optuna
...
test:
    uv run pytest tests src/sklearn_optuna --doctest-modules --doctest-continue-on-failure -n auto -v
...
test-cov:
    uv run pytest --cov=sklearn_optuna --cov-report=html --cov-report=term -n auto

# Run docstring examples
test-docstrings:
    uv run pytest --doctest-modules --doctest-continue-on-failure --no-cov src/sklearn_optuna
