...
def _get_submodules(project_root):
    ...
    Scans ``src/sklearn_optuna/`` for ``.py`` files (excluding ``__init__``)
    ...
    pkg_dir = project_root / "src" / "sklearn_optuna"
...
def _get_api_name_lookup(project_root):
    ...
    for mod in _get_submodules(project_root):
        ...
        for entry in members["classes"] + members["functions"]:
            qualified = f"sklearn_optuna.{mod['module_name']}.{entry['name']}"
...
def _build_api_table_html(project_root, prefix):
    ...
    for mod in modules:
        ...
        members = _get_public_members(mod_file, pkg_dir)
        module_label = f"sklearn_optuna.{mod['module_name']}"
...
def _get_notebook_api_usage(project_root):
    ...
    Scans example notebooks for ``from sklearn_optuna.* import …``
    ...
    for notebook in sorted(examples_dir.rglob("*.py")):
        ...
        if declared is not None:
            ...
            # Extract names imported from sklearn_optuna.*
            imported_names = set()
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module and node.module.startswith("sklearn_optuna"):
                    for alias in node.names:
...
def _build_module_toc(config, current_src_path=None, prefix=None):
    ...
    for mod in modules:
        ...
        entry = {
            "title": f"sklearn_optuna.{mod['module_name']}",
            "url": page_url,
            "active": active,
            "children": [],
        }
...
def _resolve_member_identifier(name):
    ...
    ``OtherClass.build`` becomes ``sklearn_optuna.module.OtherClass.build``
...
def _link_entry(name, title, colon, entry):
    ...
    if "." in name and name.split(".", 1)[0] != "sklearn_optuna":
...
def _process_api_page_content(html, page, config):
    ...
    is_class_page = bool(re.search(r'<h3\s+id="sklearn_optuna\.', html))

    # Locate class-level content region
    h2_match = re.search(r'<h2\s+id="sklearn_optuna\.', html)
    if not h2_match:
        return html
