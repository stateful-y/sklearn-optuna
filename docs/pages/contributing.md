...
```bash
git clone https://github.com/YOUR_USERNAME/sklearn-optuna.git
cd sklearn-optuna
```
...
=== "uv run"
    ...
    ```bash
    uv run pytest --cov=sklearn_optuna --cov-report=html
    ```
...
- [Start a discussion in the repository](https://github.com/stateful-y/sklearn-optuna/discussions)
